...
class Model(object):
    ...
    def build_model_use_knowledge_matrix(self):
        '''
        ...
        analysis_in = lasagne.layers.InputLayer((self.cluster_size, self.embedding_size), input_var=Analysis)

        # Two method to compute attention ana and ques
        AttentionTypeLayer = DotAttentionLayer
        if self.attention_type == 'bilinear_attention':
            AttentionTypeLayer = BilinearAttentionLayer
        ques_ana_attention = AttentionTypeLayer([analysis_in, ques_concat], self.n_hidden * 2)
